...
export const Option: FC<SelectOptionProps> = ({ value, label, disabled, children, index}) => {
  const { onSelect, selectedValues, multiple, selectedValue } = useContext(SelectContext)
  const isSelected = multiple ? selectedValues.includes(value) : value === selectedValue
  const classes = classNames('al-select-item', {
